...
func (t Training) Calories() float64 {
	return 0 // будет переопределяться
}
...
type Running struct {
	Training // встраивание структуры Training
}
...
func (w Walking) Calories() float64 {
	meanSpeed := w.meanSpeed() * KmHInMsec // перевод в м/с
	return ((CaloriesWeightMultiplier*w.Weight + (meanSpeed*meanSpeed/(w.Height/CmInM))*
...
func ReadData(training CaloriesCalculator) string {
	...
	info.Calories = calories // добавляем полученные калории в структуру
